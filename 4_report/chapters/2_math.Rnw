...
\ref{topology} of the appendix provides some fundamental definitions for to make 
clear how these are understood in the remainder of this report.
\\

\begin{minipage}[b]{0.7\textwidth}
  \textbf{Manifolds.} A $d$-dimensional \textit{manifold} $\mani \subset \RD$ 
  is a topological space with some additional properties.
  $\mani$ is most easily imagined as the $d$-dimensional generalization of a 
  curved surface that behaves locally Euclidean, i.e., is locally homeomorphic 
  to an open subset of $\Rd$ (\citet{mafu2011}; please refer to the appendix for 
  ...
  Clearly, the S-curve as a whole is far from linear, but it locally 
...
Put simply, \textit{geodesic distance} between two points on $\mani$ is the 
length of the shortest curve (\textit{geodesic}) between them lying on $\mani$.
...
observed in $\RD$ but assumed to be really samples from a $d$-manifold $\mani$ 
embedded in $\RD$, meaning they can be treated as $d$-dimensional, provided a 
faithful translation between $\mani$ and $\Rd$ is found\footnote{
...
$\N \ni d \ll D$, such that $\X$ is in fact a sample from a smooth, 
connected $d$-manifold with $\X \sim \mani \subset \RD$.
$\mani$ may be described by a single coordinate chart 
...
methods will be denoted by $\Y \in (\Rd)^N$ to avoid overloading
notation.