...
This has important consequences for manifold learning: data lying on a 
...
such that $\X$ is in fact a sample from a $d$-dimensional, Riemannian and 
connected manifold: $\X \sim \mani \subset \RD$.
...
i.e., compute $\Y = (\y_1, \y_2, ..., \y_N)$, with 
$\y_i \in \R^m$ $\forall i \in \setN$.
Appropriateness of $\Y$ is determined by the extent to which the 
$d$-dimensional representation preserves intrinsic structures on $\mani$.
The underlying assumption is that of a smooth mapping $\rho: \Y \rightarrow \RD$ 
with $\rho(\Y) = \X$ \citep{sudderth2002}.
\\
