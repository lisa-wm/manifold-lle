...
\textbf{Given.} Data $X = (X_1, X_2, ..., X_N)$, with $X_i \in \RD$ 
$\forall i \in \setN$ and $N, D \in \N$. 
$X$ thus consists of $N$ real-valued data vectors observed in $D$ dimensions.
...
i.e., compute $Y = (Y_1, Y_2, ..., Y_N)$, with $Y_i \in \R^d$  
$\forall i \in \setN$.
Appropriateness of $Y$ is determined by the extent to which the $d$-dimensional 
representation preserves intrinsic neighborhood structures on $\mani$.
\\
