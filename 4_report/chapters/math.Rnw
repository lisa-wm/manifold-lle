...
% TODO Mark 2 points on S-curve and connect via Euclidean distance vector
...
\begin{minipage}[b]{0.45\textwidth}
  \begin{figure}[H]
    ...
    \includegraphics[trim = 100 20 70 30, clip, % left bottom right top
      width = 0.55\textwidth]{figures/s-curve}
...
deceptively close to points in the cyan mid area), it makes intuitive sense to 
...
Building on the above concepts, the goal of manifold learning shall be 
formalized in a way that will be referenced throughout the remainder of this 
report and is inspired by \citet{sauletal2006}.
\\

\textbf{Given.} Data $\bm{X} = (\bm{x}_1, \bm{x}_2, ..., \bm{x}_N)$, with 
$\bm{x}_i \in \RD$ $\forall i \in \setN$ and $N, D \in \N$. 
$\bm{X}$ thus consists of $N$ real-valued data vectors observed in $D$ 
dimensions.
The true data-generating process is taken to have dimensionality $d \in \N$, 
such that $\bm{X}$ is in fact a sample from a $d$-dimensional, Riemannian and 
connected manifold: $\bm{X} \sim \mani \subset \RD$.
For manifold learning methods to yield satisfying results, $\mani$ is always 
assumed to be sampled well by $\bm{X}$.
\\

\textbf{Goal.} Find an appropriate $d$-dimensional representation of the data, 
i.e., compute $\bm{Y} = (\bm{y}_1, \bm{y}_2, ..., \bm{y}_N)$, with 
$\bm{y}_i \in \R^d$ $\forall i \in \setN$.
Appropriateness of $\bm{Y}$ is determined by the extent to which the 
$d$-dimensional representation preserves intrinsic structures on $\mani$.
\\

Note that, while $D$ is specified a priori, the \textit{intrinsic 
dimensionality} $d$ is typically unknown and must be estimated during learning.
Since there are not always (finite-sample) guarantees for finding the true value 
of $d$, it must be assumed that the estimation may differ from $d$.
In order to emphasize this potential gap, the dimensionality of the 
low-dimensional representation will henceforth be denoted by $m \in \N$, such
that $\bm{Y} \in (\R^m)^N$. 
