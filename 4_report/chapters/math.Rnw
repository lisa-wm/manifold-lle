...
homeomorphic to an open subset of $\Rd$, $S$ is \textit{locally 
Euclidean}\footnote{
...
shortest curve (\textit{geodesic}) on $\mani$ between two points on $\mani$ as 
...
coordinates in $\RD$ and $\Rd$ is found.
