...
\textbf{Goal.} Find an appropriate $m$-dimensional representation of the data, 
i.e., compute $\bm{Y} = (\bm{y}_1, \bm{y}_2, ..., \bm{y}_N)$, with 
$\bm{y}_i \in \R^m$ $\forall i \in \setN$.
...
low-dimensional representation is denoted by $m \in \N$, such that 
$\bm{Y} \in (\R^m)^N$. 
