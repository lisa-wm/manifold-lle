...
Geodesic distance is the length of the shortest curve (\textit{geodesic}) 
between two points $\pv, \qv \in \mani$ (such a curve must exist due to 
connectedness).
...
Euclidean spaces where shortest curves are but straight lines.
