...
In order to provide some background, this chapter will first sketch the 
fundamental idea of local graph-based techniques.
...
It is widely assumed that LLE may also be cast into this framework by 
...
H-LLE neatly fits the same idea as it bears strong algorithmic resemblance to 
...
Laplace-Beltrami operator might be subject to additional regularization 
...
The techniques studied here define functionals over smooth mappings from the 
manifold to Euclidean space and use these functionals to find optimal 
embeddings.
