...
To this end, vicinities are expressed via graph representations, where inputs 
form vertices and edges indicate neighborhood relations 
\citep{sauletal2006}.
...
established in the Euclidean space $\bm{X}$ has been observed in and are as such 
of linear nature.
