...
\begin{tight_enumerate}
  ...
  \item Construct a functional on the manifold that is associated with the 
  intrinsic data structure and estimate the functional from the graph.
  \item Learn the low-dimensional coordinates by eigenanalysis of the estimated 
  matrix.
\end{tight_enumerate}
...
\begin{minipage}[b]{0.5\textwidth}
  $\mani$ may now be characterized by a \textit{neighborhood 
