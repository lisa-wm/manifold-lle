...
\begin{itemize}
  ...
  matrix $\bm{K} \in \R^{N \times N}, \bm{K}_{ij} = \kappa(\x_i, \x_j)$.
  ...
  $\mathcal{F}$.
  Algorithmically, it boils down to diagonalizing the kernel matrix $\bm{K}$ 
...
This information is stored in a matrix representation $\bm{M}$ that is 
directly linked to the kernel matrix $\bm{K}$.
...
\begin{tight_enumerate}
  ...
  a matrix representation $\bm{M}$ thereof.
  \item Find the eigenvalues and associated eigenvectors of $\bm{M}$.
...
of the matrix $\bm{M}$ believed to hold information about the intrinsic 
manifold structure.
...
$\lambda \bm{I} - \bm{A}$ 
(with $\bm{I}$ the $N$-dimensional identity matrix) constitutes the 
...
$\bm{B} = \bm{I}$, the generalized form subsumes both cases.
It is given by 
...
\begin{equation}
  \max_{\bm{V}} \text{\textit{trace}}(\bm{V}^T \bm{A} \bm{V}), \quad \text{s.t.} 
  \quad \bm{V}^T \bm{B} \bm{V} = \bm{I},
