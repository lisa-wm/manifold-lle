...
to $\x_i \in \X$ for $i \in \setN$.
Let $\bm{N}_i \in \R^{D \times k}$ denote the matrix of feature vectors of
$\x_i$'s neighbors that have been centered with respect to the mean over all 
...
For a more in-depth analysis that also encompasses examination of limit behavior 
and the connection between HLLE and \textit{local tangent space alignment 
(LTSA)} see, for example, \citet{tingjordan2018}.
}.
