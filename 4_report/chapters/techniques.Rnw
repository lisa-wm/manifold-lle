...
Now, defining the graph Laplacian as 
$\bm{L} = \bm{D} - \bm{W} \in \R^{N \times N}$, the above can be rewritten as
...
\ref{eq-gevproblem} \citep{belkinniyogi2003}.
As in the continuous case, the bottom eigenvector is constant and must be 
discarded; the subsequent $d$ eigenvectors yield the desired low-dimensional 
...
% In LEM, these weights are computed using a heat kernel\footnote{
% As an alternative, \citet{belkinniyogi2003} propose a simpler kernel that is 
% equal to 1 for connected nodes and 0 otherwise.
% }, such that 
% $w_{ij} = \exp(\frac{\|x_i - x_J \|^2}{t})$, $t \in \R$, for any two points 
% $x_i, x_j \in \RD$ if they are connected by a graph edge, and 0 otherwise.
% Clearly, edges between closer points receive larger weights.
