...
Algorithmically, LLE then performs two subsequent steps \citep{roweissaul2000}:
...
By standard use of a Lagrange multiplier, the solution for the above constrained optimization problem eventually collapses to:
...
This can now be expressed as a standard eigenvalue problem.
For this purpose, define $\E = (\I - \W)^T(\I - \W)$ and set 
$\tilde{\Y} = \Y^T$ \citep{cayton2005}, such that:

\begin{equation}
  \begin{split}
    & \min_{\tilde{\Y}} \text{\textit{trace}}(\tilde{\Y}^T \E \tilde{\Y}) \\
    & \text{s.t. } \tilde{\Y}^T \tilde{\Y} = \I, \quad \text{and} \\
    & \phantom{\text{s.t. }} \sum_i \y_i = \bm{0} \quad \forall i \in \setN.
  \end{split}
  \label{eq-obj-lle-emb-2}
\end{equation}

The close resemblance to the optimization problem in LEM (equation 
\ref{eq-obj-lem}) is not coincidental.
...
Again, the solution is thus found by eigenanalysis of the matrix encoding the 
intrinsic manifold structures, i.e., by computing the bottom $d$ eigenvectors 
of $\E$ after discarding the bottom one associated with a zero eigenvalue \citep{cayton2005}.
