...
\begin{figure}[H]
 ...
 \begin{subfigure}[b]{0.58\textwidth}
   ...
   % \caption{Reconstruction in three dimensions}
 \end{subfigure}
 \hfill
 \begin{subfigure}[b]{0.4\textwidth}
   \centering
   \includegraphics[width = \textwidth]{figures/reconstruction-2d}
   % \caption{Reconstruction in two dimensions}
 \end{subfigure}
  \caption{Reconstruction in three (\textit{left}) and two (\textit{right}) 
  dimensions, employing the same reconstruction weights. \textit{Source:} own
  representation.}
  \label{fig:three graphs}
\end{figure}

The LLE algorithm consists of two subsequent steps \citep{roweissaul2000}:
