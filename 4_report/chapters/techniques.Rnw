...
quadratic functional $\mathcal{H}(f)$ on the entire manifold, analogous to the 
Laplace-Beltrami operator in LEM.
The crucial property of $\mathcal{H}(f)$ is given by the fact that, if $\mani$ 
is truly locally homeomorphic to an open, connected subset of $\Rd$, 
$\mathcal{H}(f)$ has a $(d + 1)$-dimensional null space.
...
Thus, $\mathcal{H}(f)$ as a measure for overall curviness of the mapping is 
given by \citep{donohogrimes2003}:

\begin{equation}
  \mathcal{H}(f) = \int_{\mani} \frobnorm{\Hes_f^{\text{loc}}(\pv)} d\pv.
  \label{eq-hfunctional}
\end{equation}

\textbf{HLLE.}
In analogy to LEM, the functional defined on $\mani$ must be approximated in an 
empirical manner.
