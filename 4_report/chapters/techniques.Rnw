...
A second matrix $\D = (d)_{ij} \in \R^{N \times N}$ takes the row sums of
$\W$ on its diagonals.
...
$\Lap = \D - \W \in \R^{N \times N}$, thereby coercing all information 
...
closed-form solution \citep{roweissaul2000}:

\begin{equation}
  \begin{split}
    \min_{\W} \varepsilon(\W) &= \min_{\W} \sum_i \| \x_i - \sum_j w_{ij} \x_j 
    \|^2_2 \\
    &= \min_{\W} \sum_i \| \x_i - \bm{N}_i \bm{w}_i \|^2_2
  \end{split}
  \label{eq-obj-lle}
\end{equation}
...
Equation \ref{eq-obj-lle} can be re-arranged by use of the sum-one constraint, 
implying $\bm{1}^T \bm{w}_i = \bm{1}$, and simplified by introduction of the 
Gram, or local covariance, matrix $\G_i$ 
\citep{saulroweis2001}:

\begin{equation}
  \begin{split}
    \min_{\W} \varepsilon(\W) &= \min_{\W} \sum_i \| \x_i \bm{1}^T \bm{w}_i - 
    \bm{N}_i \bm{w}_i \|^2_2 \\
    &= \min_{\W} \sum_i \| (\x_i \bm{1}^T - \bm{N}_i) \bm{w}_i \|^2_2 \\
