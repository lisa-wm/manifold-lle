...
\begin{minipage}[b]{0.6\textwidth}
  \textbf{Idea.}
  All of the above methods operate in a fully unsupervised manner, relying 
  ...
  \citet{yangetal2006} propose to anchor the low-dimensional representation at a 
  number $m \in \N$ of prior points whose coordinates in $\Rd$ are already 
  known.
...
\end{subfigure}%
  ...
  \caption[S-curve with poor, random and optimal landmark coverage]{
    ...
    (\textit{right}) coverage (again, 12 prior points).
    \textit{Source:} own representation.}
...
While the first case greatly simplifies of the manifold learning problem, the 
second must be treated with some more care, or the semi-supervision will 
actually be harmful rather than beneficial.
...
different nature: now, the matrices $\E$ and $\tilde{\Y}$ are partitioned into 
...
This leads to a system of linear equations whose solution depends on whether 
prior information is exact.
If it is exact, the minimization problem collapses to optimizing over the 
unknown coordinates, denoted by $\Ytil_2$ \citep{yangetal2006}:
...
above: as errors exist only in $\E_{12} = \E_{21}$ and $\E_{22}$, the condition 
...
deviations of the assumed coordinates $\hat{\Ytil}_1$ from the true ones.
