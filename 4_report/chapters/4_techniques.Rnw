...
LEM (in fact, it was proposed under the title \textit{Hessian eigenmaps}) 
but is akin to LLE in an algorithmic sense\footnote{
...
connected subset of $\Rd$, which is a relatively weak demand 
\citep{donohogrimes2003}.
With this, HLLE provides veritable convergence guarantees for a wide range of 
cases, albeit only for the continuum limit \citep{cayton2005}.
\\
...
It is easy to conceive practical applications where prior
...
Other applications like, say, textual analysis might not reveal this
information at first sight, but can benefit from pre-labeled data made
available by the advance of open-source research.
}.
...
\begin{figure}[H]
  \begin{subfigure}[b]{0.25\textwidth}
    \includegraphics[trim = 80 20 60 0, clip, % left bottom right top
      width = 0.9\textwidth]{figures/s_curve_pp_poor}
  \end{subfigure}%
  \begin{subfigure}[b]{0.25\textwidth}
    \includegraphics[trim = 80 20 60 0, clip, % left bottom right top
      width = 0.9\textwidth]{figures/s_curve_pp_random}
\end{subfigure}%
  \begin{subfigure}[b]{0.25\textwidth}
    \includegraphics[trim = 80 20 60 0, clip, % left bottom right top
      width = 0.9\textwidth]{figures/s_curve_pp_maxmin}
  \end{subfigure}
...
ones.
The associated regularization parameter $\beta \in \R$ encodes the level of 
...
Both factors will be varied and and examined in the practical experiments.
Likewise, the reliability of prior information ought to be a critical factor in 
SSLLE.
...
Chapter \ref{experiment} studies the impact of label contamination simulated by 
...
\citep{sauletal2006}.
For LLE, LEM and HLLE, no such tell-tale gap exists\footnote{
While \citet{shasaul2005} have drawn a mathematical relation between the 
respective eigenspectra in LLE and LEM and intrinsic dimensionality, they 
...
This procedure builds upon a proposal by \citet{kouroptevaetal2001}.
...
The key idea is to select $k$ at minimum computational expense, most of which is 
...
For the most promising of these, distinguished as local minima of the resulting reconstruction error distribution\footnote{
While weight reconstruction for a fixed $k$ is a convex optimization problem, 
finding the optimal value for $k$ is not.
} (i.e., whenever the error is lower for a certain $k$ 
than for the immediate successor and predecessor values), the actual embeddings 
are calculated and evaluated to select the optimal parameter value
...
minimization in (SS)LLE frequently suffers from singularity (in particular, for 
$D < k$). 
...
The focus of this report lies on the challenges posed by the 
semi-supervised extension, so the issue is not specifically addressed, but it 
would be certainly worthwhile to dedicate some more research to this.