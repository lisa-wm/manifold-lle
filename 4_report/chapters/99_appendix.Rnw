...
\begin{itemize}
  \item Area under the $R_{NX}$ curve
  \item Based on co-ranking matrix
\end{itemize}
...
\begin{itemize}
  \item Rank distance matrices $(r)_{ij}^{\text{obs}}, (r)_{ij}^{\text{emb}} 
  \in \R^{N \times N}$
  \item Co-ranking matrix $\bm{Q} = (q)_{\ell m} \in \R^{N \times N}$ with 
  $q_{\ell m} = 
  \big \rvert \{ (i, j): 
  r_{ij}^{\text{emb}} = \ell \land r_{ij}^{\text{obs}} = m \} \big \rvert$ 
  \item Interpretation:
  \begin{itemize}
    \item All non-zero entries on diagonal $\Rightarrow$ optimal 
    embedding
    \item Most non-zero entries on upper triangle $\Rightarrow$ close 
    points torn apart
    \item Most non-zero entries on lower triangle $\Rightarrow$ faraway 
    points collapsed
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item Number of points remaining in $k$-neighborhood after projection: 
...
\begin{minipage}[b]{0.6\textwidth}
  ...
  \begin{itemize}
    \item $\text{AUC}(R_{NX}) = \dfrac{\sum_{k = 1}^{N - 2} R_{NX}(k)}{
    \sum_{k = 1}^{N - 2} 1 / k} \in [0, 1]$
    \item Interpretation:
    \begin{itemize}
      \item $\text{AUC}(R_{NX}) = 0$ $\Rightarrow$ random embedding
      \item $\text{AUC}(R_{NX}) = 1$ $\Rightarrow$ optimal embedding
    \end{itemize}
\end{itemize}
\end{minipage}%
