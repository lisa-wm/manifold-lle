...
proposal of LLE lacks, and HLLE emerges as a combination of both ideas.
...
\textbf{kPCA.}
As indicated in figure \ref{fig_models_overview}, LGML may be viewed in the 
light of \textit{kernel principal component analysis (kPCA)}. %see \ref{kpca}).
kPCA was actually proposed earlier and later shown to link the other 
concepts by a unified idea (\citet{hametal2003}.
It provides a useful general intuition to manifold learning and subsumes the 
other methods in a way beneficial to the important task of out-of-sample 
...
\textit{kernel trick}.
First, features of interest are extracted from the data by kernelization and 
coerced to a matrix representation. 
...
vary by eigenanalysis, yielding the desired reduction in dimensionality through 
preserving the most relevant latent dimensions \citep{schoelkopfetal1998}.

% ------------------------------------------------------------------------------

\subsection{Concept of LGML}
\label{lgml_concept}
...
to finding linear embedding spaces \citep{vandermaatenetal2009}.
As $\X$ must generally be assumed to lie on a non-linear manifold, 
kernelization is usually performed with non-linear feature maps 
...
able to capture arbitrarily structures.
Distances may be then measured along the approximated manifold surface rather 
than in the ambient Euclidean space, effectively enabling non-linearity 
\citep{sauletal2006}.
Besides non-linearity, a second desideratum in manifold learning is the ability 
to treat possibly non-convex manifolds with sufficiently local focus.
Non-convexity means $\mani$ is not isometric to a convex subset of Euclidean 
space \citep{donohogrimes2003}. 
Intuitively, such behavior requires careful tracing of the manifold surface.
LGML methods therefore focus primarily local manifold properties\footnote{
...
high-dimensional observation space, where neighborhoods are typically taken to 
be $k$-neighborhoods, i.e., based on a fixed number of neighbors.
In principle, it is equally possible to restrict neighborhoods to a maximum 
distance.
However, $k$-neighborhoods are often more easily specified due to the inherent 
scale invariance of $k$, and has attracted rather more attention in general research\footnote{
...
For a formal definition of both $k$- and $\epsilon$-neighborhoods, see section
\ref{neighborhoods}.
Neighborhood size represents a crucial parameter in LGML.
It reflects beliefs about the topological structure of $\mani$ -- 
...
Chapter \ref{experiment} will discuss how this trade-off is addressed in 
practice.

% \textbf{Graph construction.}
% $\mani$ may be approximated by a \textit{neighborhood graph} 
...
\begin{minipage}[b]{0.65\textwidth}
  ...
  Figure \ref{fig-neighbor-graph} shows how a neighborhood graph may be used to 
...
\begin{minipage}[b]{0.3\textwidth}
  \begin{figure}[H]
    ...
    \includegraphics[trim = 250 170 200 140, clip, % left bottom right top
      width = 0.7\textwidth]{figures/s_curve_connected}
...
Precisely how the functional is defined reflects different beliefs about the 
intrinsic manifold structure and defines the core of each LGML method 
\citep{sauletal2006}.
The $d$ principal (top or bottom) eigenvectors -- as determined by the 
...
The nature of different matrix representations across methods shall be made 
clear in the subsequent chapters.
Figure \ref{fig_kpca_lgml_algo} concludes the overview on LGML with a summary 
...
\begin{figure}[H]
  ...
  \caption[LGML concept]{Concept of LGML in the kPCA framework.
  \textit{Source:} own representation.}
  \label{fig_kpca_lgml_algo}
\end{figure}