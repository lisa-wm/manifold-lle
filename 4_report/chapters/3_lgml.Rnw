...
able to capture arbitrary structures.
...
to handle (possibly non-convex) manifolds with complicated surfaces.
Non-convexity means $\mani$ is not isometric to a convex subset of Euclidean 
space \citep{donohogrimes2003}. 
Intuitively, such behavior requires careful tracing of the manifold surface to 
avoid coarse mappings of the global structure at the expense of local 
congruence.
LGML methods therefore focus on local properties\footnote{
...
distance of $\epsilon \in \R^+$ to the centroid.
...
\ref{neighborhoods}.
Both notions usually rely on Euclidean distance thanks to the manifold's locally 
linear behavior.
In the end, any vicinity condition is admissible so long as it serves to 
faithfully characterize the manifold surface and construct neighborhoods in a 
computationally affordable manner \citep{roweissaul2000}.
For the remainder of this report, neighborhoods will be taken to be 
$k$-neighborhoods.
The crucial parameter for LGML is, in fact, neighborhood size.
It reflects beliefs about the topological structure of $\mani$: smaller 
neighborhoods correspond to a higher degree of non-linearity, emphasizing local 
properties more strongly, and vice versa.
The choice of $k$ therefore has a rather strong impact on performance 
\citep{sudderth2002}.
Chapter \ref{experiment} will discuss how the trade-off between global and local 
structure preservation is addressed in practice.
% \\
...
\begin{minipage}[b]{0.25\textwidth}
  \begin{figure}[H]
    \centering
    \includegraphics[trim = 250 170 200 160, clip, % left bottom right top
      width = 0.75\textwidth]{figures/s_curve_connected}
...
Precisely how the functional is constructed defines the core of each LGML method 
\citep{sauletal2006}.
...
\begin{figure}[H]
  \centering
  \includegraphics[trim = 0 0 0 0, clip, % left bottom right top
    width = 0.8\textwidth]{figures/kpca_lgml_algo}
  \caption[LGML concept]{
  \raggedright
  Concept of LGML in the KPCA framework.
  \textit{Source:} own representation.}
