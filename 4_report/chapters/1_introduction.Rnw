...
neighborhood graphs.
They pay particular heed to local environments and are thus able to retrieve 
...
relations.
\textit{Laplacian eigenmaps (LEM)} was ceveloped somewhat later on a more rigid 
...
an open-source implementation, and (2) to assess its 
