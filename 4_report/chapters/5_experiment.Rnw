...
\textbf{Sensitivity analysis I: landmark coverage versus number of prior 
points.}
The first experiment pits the quality of landmark coverage against the number of 
landmarks, reflecting the hypothesis of optimal landmark choice dominating 
...
\textbf{Sensitivity analysis II: label noise versus number of prior 
points.}
...
Variation is steered via noise level $\alpha$.
...
Embedding quality is evaluated on the area under the $R_{NX}$ curve, 
$\text{AUC}(R_{NX})$, as proposed by \citet{kraemeretal2019}.
This metric assesses the extent to which neighborhood relations are preserved. 
The $R_{NX}$ curve results from a normalized count of points that, for all 
...
will therefore be also inspected visually, which is possible here thanks to the 
...
each.
The \textit{Swiss roll} data is a widely used two-manifold resulting from 
rolling a rectangular patch of data up to a spiral-like shape and embedding it 
in $\R^3$.
A somewhat more complex shape results from bending that same rectangular patch 
to form an \textit{incomplete tire}, basically a hollow torus with a strip and 
a chunk cut out from the tube in longitudinal and cross-section, respectively.
...
observations, is employed for the concluding comparison of SSLLE, LLE and HLLE 
in chapter \ref{comparison}.
It represents a simplified model of the Earth with five continents (omitting the 
poles).
...
\begin{figure}[H]
  ...
  (\textit{left}), incomplete tire (\textit{middle}), and world data sets.
...
\subsection{Results of the Sensitivity Analysis}
\label{results}
...
\begin{minipage}[b]{0.4\textwidth}
  ...
  While poor and random coverage are identical for examples, optimal 
...
An important thing to note here is that the points do not appear to actually
cover the surface well: they mostly cluster on the fringes of the embedding.
This artifact must probably be attributed to the estimation of geodesic 
distance for which the implementation relies the \texttt{dimRed} 
package \citep{pkgdimred}.
\\

\textbf{Quantitative results.}
Performance across scenarios reflects this observation.
...
\begin{figure}[H]
  ...
  \raggedright
  Quantitative results for method of landmark coverage versus number of prior 
  points. 
...
points, particularly so for the Swiss roll data.
The picture is somewhat more balanced for the incomplete tire.
...
\begin{minipage}[b]{0.4\textwidth}
  ...
  label noise in order to simulate inexact prior information.
...
Figure \ref{fig_key_noise} illustrates the potential impact this entails.
...
\textbf{Qualitative results.}
Again, the actual embeddings are depicted below to support the evaluation.
The minimum amount of two landmarks is insufficient in any case.
Above that, SSLLE embeds the incomplete tire fairly well for low levels of 
noise, while at least six prior points seem necessary to find an embedding of 
the Swiss roll that actually resembles the true one.
...
Comparison on both data sets (figure \ref{fig_comp_swiss_tire}) show SSLLE is a 
strong contender against the other two methods.
LLE does not find a meaningful embedding in either case, while HLLE 
performs impressively on the Swiss roll data but fails to fully undo the 
incomplete tire transformation.

\begin{figure}[H]
  \begin{subfigure}{0.5\textwidth}
    \centering
    \includegraphics[trim = 60 0 20 0, clip, width = 0.9\textwidth]
  ...
  data]
  {\raggedright
  Embeddings found by LLE (\textit{left}), HLLE (\textit{middle}), and SSLLE 
  (\textit{right}).
  \textit{Source:} own representation.}
  \label{fig_comp_swiss_tire}
\end{figure}

The somewhat more complicated world data manifold (figure \ref{fig_comp_world}), 
which contains multiple disconnected components, discriminates between 
successful and failed embeddings in an even more striking manner. 
...
\begin{minipage}[b]{0.45\textwidth}
  ...
  Note that 25 landmarks have been used this time to achieve a comparable share 
  of prior points with respect to total observations.
\end{minipage}%
\begin{minipage}[b]{0.05\textwidth}
  \phantom{xxx}
\end{minipage}%
