...
after all and might be considered superior to the randomly selected prior 
...
\begin{figure}[H]
  ...
  {\raggedright
  Qualitative results Swiss roll data, evaluating method of landmark coverage 
...
\begin{minipage}[b]{0.55\textwidth}
  \begin{figure}[H]
    \begin{subfigure}[c]{\textwidth}
      \centering
      \includegraphics[trim = 50 20 0 20, clip, % left bottom right top
    ...
    \raggedright
    Varying levels of noise exemplified in one prior point location.
    Semi-axes of ellipsoids indicate standard deviation in the respective 
    direction.
    \textit{Source:} own 
    representation.
    }
