...
Embedding quality is evaluated on the \textit{area under the $R_{NX}$ curve}, 
...
\begin{figure}[H]
  ...
  \raggedright
  Quantitative results for noise level versus number of prior points. 
  $\text{AUC}(R_{NX})$ has been scaled to take on a minimum of 0 and maximum of
  1 in both figures for better visibility of differences.
...
\begin{figure}[H]
  ...
  {\raggedright
  Qualitative results for Swiss roll data, evaluating noise level versus number 
  of prior points.
  \textit{Source:} own representation.}
...
\begin{figure}[H]
  ...
  II]
  {\raggedright
  Qualitative results for incomplete tire data, evaluating noise level versus 
