...
\pagestyle{fancy}
\fancyhead{}
\fancyhead[R]{\changefont{Applying Semi-Supervised Locally Linear Embedding}}
\fancyfoot{}
\fancyfoot[R]{\thepage}
...
\noindent
\normalsize
\input{chapters/extended-abstract.Rnw}
\newpage
...
\begin{tabularx}{\textwidth}{ 
  ...
  $X = (X_1, X_2, ..., X_N) \in (\RD)^N$ & Observed coordinates of data sampled 
  from $\mani$ \\
  $Y = (Y_1, Y_2, ..., Y_N) \in (\Rd)^N$ & Learned coordinates of low-dimensional
...
\input{chapters/graph-based-models.Rnw}
