...
representation.
With $\Lap$ the above can be rewritten as a generalized eigenvalue problem in
the LGML conceptual spirit:
...
origin.
For $\pv^{\prime} \in \mathcal{N}_k(\pv)$, let these local coordinates be 
denoted by $\x^{\text{loc, }\pv^{\prime}}$.
...
\begin{equation}
  ...
  \bigg\rvert_{\x^{\text{loc, }\pv^{\prime}} = 0} \quad 
  i, j \in \{ 1, 2, \dots, d \}.
...
\begin{equation}
  \mathcal{H} = \sum_{i,j} \mathcal{H}_{ij}, \quad 
  \mathcal{H}_{ij} = \sum_{\ell, m} (\Hes_{\ell})_{m,i}
  (\Hes_{\ell})_{m,j}, \quad i, j \in \setN.
...
to address the issue in detail, it would be certainly worthwhile.